...
- `scripts/scaffold.py` — Core scaffolding engine. Generates commands, skills, hooks, MCP configs, settings.json
- `scripts/audit.py` — Config auditor. Scores any .claude/ setup for health, tokens, staleness
- `reference/` — Stack-specific knowledge (stacks.md, agents.md, audit-rules.md)
- `templates/` — Lean templates for CLAUDE.md, agents, ARCHITECTURE.md
...
- MCP env vars use `${VAR}` syntax, never hardcoded secrets
...
scripts/test_*.py      — Test suites (100+ tests)
...
Key test areas: command injection blocking, hook scoping, settings merge, dry-run mode, staleness detection, secret detection.
