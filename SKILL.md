...
The scaffold writes `settings.json` (hooks + `statusLine` + `fallbackModel`) and a
project-root `.mcp.json` (`{"mcpServers": {...}}`, the team-shared, version-controlled
convention — MCP servers do NOT go in settings.json). Now customize:
