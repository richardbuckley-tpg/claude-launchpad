...
description: >
  ...
  a complete .claude/ configuration — CLAUDE.md (≤100 lines), 11-16 agents (≤30 lines each), path-scoped
  rules, hooks, 9-15 skills, 17-22 slash commands, MCP config, and session handoff — all with real values,
...
**Model & effort** (see `reference/agents.md` → Model & Effort Selection): agents use the
`opus`/`sonnet` aliases (Opus 4.8 / Sonnet 4.6 today). The architect runs `effort: xhigh`
(deepest persisted reasoning, Opus 4.7+); tune `effort` (`low`→`medium`→`high`→`xhigh`→`max`)
and `model` per agent to trade depth vs. cost. Route mechanical agents to `model: haiku`.
For whole-repo agents on large codebases, the `opus[1m]`/`sonnet[1m]` 1M-context aliases exist.

Agents: architect, testing, reviewer, debugger, refactorer, docs-generator, push, idea-to-prd, pre-push, dev-ops (always) + security (when auth/payments/user data) + reliability-auditor (when event systems detected)
...
The scaffold writes `settings.json` (hooks + `statusLine` + `fallbackModel: "sonnet"` +
a `permissions` block) and a project-root `.mcp.json` (`{"mcpServers": {...}}`, the
team-shared, version-controlled convention — MCP servers do NOT go in settings.json).

**Permissions** (generated by default): `permissions.deny` blocks reads of `.env`,
`secrets/**`, `*.pem`/`*.key`, `~/.ssh`, and cloud-credential files (OS-independent —
stronger than `.claudeignore` for the Read tool). `permissions.allow` pre-approves the
project's own validated test/lint/build/dev commands so routine runs don't prompt;
migrations go in `ask`. (`exit 2` security hooks are kept — they're the *unbypassable*
block; `permissionDecision: deny` is weaker, so don't swap them.)

**Sandbox** (opt-in, `--sandbox`): generates a `sandbox` block (OS-enforced Bash
isolation, macOS/Linux/WSL2) with credential-dir `denyRead` and stack-aware
`allowedDomains` for prompt-free autonomous `/build` runs. Suggest it for users who run
unattended. Tell users they can also toggle it live with `/sandbox`.

**Memory**: write-capable agents (architect/debugger/refactorer) get `memory: project`
for cross-session learning. Claude Code also has native **auto-memory** (it maintains its
own `MEMORY.md`) — distinct from author-written CLAUDE.md and from Launchpad's `/learn`.
For lasting facts, prefer CLAUDE.md (you write) or `/learn`; let auto-memory accrue
incidental notes. See `/memory`.

Now customize:
- **Add hooks** from the interview (auto-lint, auto-test, force-push block, conventional commits)
- **Verify MCP servers** in `.mcp.json` — secrets in `env`/`headers` must be documented in `.env.example`
- GitHub & Sentry use official **remote HTTP servers** (`type: http`); Postgres uses Postgres MCP Pro
  (`crystaldba/postgres-mcp`) via Docker. Deprecated `@modelcontextprotocol/server-*` packages are not used.
  No SQLite MCP is generated (the reference server is archived/vulnerable).
- **Add any extra MCP servers** the user requested beyond auto-detected ones
- `settings.json` may also carry current keys like `statusLine`, `outputStyle`, `modelOverrides`,
  `requiredMinimumVersion`, `attribution`, and `disableWorkflows` — all recognized by the auditor.
...
The full idea-to-ship pipeline:

```
/idea-to-prd "feature idea"   →  PRD in docs/prds/ (optional first step)
...
5. **Cleanup** — remove dead code, debug artifacts, test slop, over-defensive checks
6. **Domain Audit + Review** (parallel when domain set) — both must approve
7. **Pre-push** — lint, test, build, secrets, debug code scan
8. **Ship** — creates PR, cleans up worktree
...
The `@dev-ops` agent is available independently for infrastructure planning — run it to get Docker Compose + Terraform starters with cost estimates.
